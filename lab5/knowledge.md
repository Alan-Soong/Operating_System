...
**用户程序如何在操作系统里面跑起来：**
需要编译器把用户程序的源代码编译为可以在CPU执行的目标程序，这个目标程序里，既要有执行的代码，又要有关于内存分配的一些信息。给它分配一些内存，把程序代码加载进来，建立一个进程，然后通过调度让这个用户进程开始执行。

**用户程序如何安全地获取操作系统服务？**
系统调用正是连接用户态与内核态的桥梁。它为用户程序提供了一套标准化的服务接口，使得用户程序能够通过受控的方式使用内核功能。  
ecall 指令会触发一个异常事件，使 CPU 从用户态提升到内核态，并跳转到预设的中断处理程序 trap 中，在其中层层转发到系统调用函数 write 进行处理，之后再通过 sret 指令返回到用户态。
