...
<img src="./OS_lab1.assets/image-20251007192058616.png" alt="image-20251007192058616" style="zoom:30%;" />

在进入之后，我们可以查看一下相关的情况：

<img src="./OS_lab1.assets/image-20251007191747637.png" alt="image-20251007191747637" style="zoom:30%;" />
...
<img src="./OS_lab1.assets/image-20251007211753681.png" alt="image-20251007211753681" style="zoom:30%;" />

此时`kern_entry`已经执行完毕了。

>  在下面的图片中，使用`tmux`可以实现终端的分屏：<img src="./OS_lab1.assets/image-20251006122437837.png" alt="image-20251006122437837" style="zoom:30%;" />
>
...
这个 bt 输出表明：系统启动成功，已正确进入内核代码；执行位置在预定的内核入口点，正确；栈状态干净，只有一个栈帧，说明是初始状态；准备进入C代码即将完成汇编初始化，跳转到C语言环境

这是内核启动过程中一个非常关键和正常的状态，说明从 OpenSBI 到内核的切换已成功完成！做完上述实验之后，我们可以回答下面的问题了：
...
内核刚启动时，没有任何运行环境，**必须要手动建立一块可用的栈空间让后续的函数调用**，局部变量分配能够正常使用栈空间，为进入`kern_init`提供所需环境  
