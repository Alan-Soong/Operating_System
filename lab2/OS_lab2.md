...
#### 1. x86 平台: BIOS/UEFI 服务
    传统 BIOS - E820 中断:
        OS 在启动初期可以调用 BIOS 的 `INT 15h` 中断，并将 `AX` 寄存器设置为 `E820h`。BIOS 会返回一个详细的内存区域描述符列表（E820 内存映射），其中详细说明了物理地址空间中每个区域的起始地址、长度和**类型**（如可用内存、保留内存、ACPI 数据等）。 OS 凭借此列表，就能精确地构建出物理内存的全貌。
